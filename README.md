...
这是当前统一后的正式基线版本。程序界面、状态栏、启动日志和 Git tag 统一使用 `v2.1.2`，版本号后不再追加运行时后缀。
...
Git 最新维护 tag 为 `v2.1.2`。如果你看到的是其他启动字符串，说明运行的不是当前统一后的基线版本。
...
当前验证结果（2026-08-02）：完整离线 pytest 回归通过，共 `485 passed`。外站 live-network 测试不作为默认阻塞项。
...
它们用于记录历史修复过程。**正式当前源码维护说明以本文档、`MAINTENANCE.md` 和 Git tag `v2.1.2` 为准。**

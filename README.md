...
这是当前统一后的正式基线版本。程序界面、状态栏、启动日志和 Git tag 统一使用 `v1.5.5`，版本号后不再追加运行时后缀。
...
Git 最新维护 tag 为 `v1.5.5`。如果你看到的是其他启动字符串，说明运行的不是当前统一后的基线版本。
...
它们用于记录历史修复过程。**正式当前源码维护说明以本文档、`MAINTENANCE.md` 和 Git tag `v1.5.5` 为准。**

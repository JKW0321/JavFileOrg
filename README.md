...
这是当前统一后的正式基线版本。程序界面、状态栏、启动日志和 Git tag 统一使用 `v2.1.1`，版本号后不再追加运行时后缀。

当前版本已经整合并验证以下能力：

1. **组件化 macOS 界面**：使用 WebView 主界面、设置页和运行报告页
2. **自动数据源路由**：支持全自动、自动有码、自动无码和指定详细来源
3. **文件名识别增强**：兼容下载站前缀、括号编号、字母后缀及多种视频组命名
4. **视频组共享封面**：普通处理与巡检均按一张封面对应一组视频维护
5. **巡检状态细化**：区分待巡检、预检中、已预检、巡检中和最终结果
6. **安全巡检**：检查异常小视频、重复副本、孤儿图片和缺失/损坏封面
7. **事务性处理增强**：视频移动后做大小校验，视频组支持事务性回滚
8. **审计能力增强**：支持 Dry Run、运行日志、before/after manifest 和 run summary
...
```text
✅ JAV 文件整理工具 v2.1.1 启动完成 | baseline-v2.1.1 | 2026-08-01
```

Git 最新维护 tag 为 `v2.1.1`。如果你看到的是其他启动字符串，说明运行的不是当前统一后的基线版本。
...
当前验证结果（2026-08-01）：完整离线 pytest 回归通过，共 `412 passed`。外站 live-network 测试不作为默认阻塞项。
...
它们用于记录历史修复过程。**正式当前源码维护说明以本文档、`MAINTENANCE.md` 和 Git tag `v2.1.1` 为准。**
